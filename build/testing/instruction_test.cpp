...
TEST (addTest, add) {
    ...
    /* ADD SP, nn */
    mem_write8(c->mem, c->regs->pc, 0x11); // LD DE nnnn
    mem_write16(c->mem, c->regs->pc +1, 0xfff8); // nnnn
    mem_write8(c->mem, c->regs->pc +3, 0xd5); // PUSH DE 
    mem_write16(c->mem, c->regs->pc +4, 0xe802); // ADD SP, 0x02
    run_cpu_loop(c);
    EXPECT_EQ((get_reg_pair(c->regs, _DE)), 0xfff8);
    EXPECT_EQ(stack_peak(c->stack), 0xfffa);
...
TEST (lastLoadTest, lastLoad) {
    ...
    EXPECT_EQ(mem_read8(c->mem, 0xbeef), 0x23);

    /* LDH A, (nnnn) */
    mem_write8(c->mem, 0xc0fe, 0x42);

    mem_write8(c->mem, c->regs->pc, 0xfa);
    mem_write16(c->mem, c->regs->pc +1, 0xc0fe);
    mem_write8(c->mem, c->regs->pc +3, 0x00);
    run_cpu_loop(c);

    EXPECT_EQ(mem_read8(c->mem, 0xc0fe), 0x42);
    EXPECT_EQ(*(get_reg(c->regs, _A)), 0x42);

}

TEST (jumpHLTest, jumpHL) {
    memory *m = new_memory();
    cpu *c = new_cpu(m);

    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL nnnn */
    mem_write16(c->mem, c->regs->pc +1, 0xc0fe); // nnnn
    mem_write8(c->mem, c->regs->pc +3, 0xe9); // jp HL
    mem_write8(c->mem, c->regs->pc +4, 0x00); // jp HL
    run_cpu_loop(c);
    EXPECT_EQ(c->regs->pc, 0xc0fe +1);
    EXPECT_EQ((get_reg_pair(c->regs, _HL)), 0xc0fe);
...
/* TODO: update all HL functions
 * to read from an address instead 
 * of reading from _HL reg directly */
/*************************/
TEST (rlcTest, rlc) {
...
TEST (slaTest, sla) {
    ...
    set_all_flags(c->regs, 0, 0, 0, 0); // Turn carry bit off 
    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +1, 0xbeef); /* LD B, 0x00ff */
    mem_write8(c->mem, 0xbeef, 0xff); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +3, 0xcb26); // SLA (HL)
    run_cpu_loop(c);
    EXPECT_EQ(mem_read8(c->mem, (get_reg_pair(c->regs, _HL))), 0x00fe);
    EXPECT_EQ(c->regs->flag, CY_MASK);
}
...
TEST (sraTest, sra) {
    ...
    set_all_flags(c->regs, 0, 0, 0, 0); // Turn carry bit off 
    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +1, 0xc0fe); /* LD B, 0x00ff */

    mem_write8(c->mem, 0xc0fe, 0x01); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +3, 0xcb2e); // SRA (HL)
    run_cpu_loop(c);
    EXPECT_EQ(mem_read8(c->mem, (get_reg_pair(c->regs, _HL))), 0x0000);
    EXPECT_EQ(c->regs->flag, CY_MASK | Z_MASK);
}
...
TEST (swapTest, swap) {
    ...
    set_all_flags(c->regs, 0, 0, 0, 0);
    mem_write16(c->mem, c->regs->pc, 0x3e8A); /* LD A, 0x8A */
    mem_write16(c->mem, c->regs->pc +2, 0xcb37); /* SWAP A */
    mem_write8(c->mem, c->regs->pc +4, 0x00); /* Exit */
    run_cpu_loop(c);
    EXPECT_EQ(*(get_reg(c->regs, _A)), 0xA8);
    EXPECT_EQ(c->regs->flag, 0x00);

    set_all_flags(c->regs, 0, 0, 0, 0);
    mem_write16(c->mem, c->regs->pc, 0x3e00); /* LD A, 0x00 */
    mem_write16(c->mem, c->regs->pc +2, 0xcb37); /* SWAP A */
    mem_write8(c->mem, c->regs->pc +4, 0x00); /* Exit */
    run_cpu_loop(c);
    EXPECT_EQ(*(get_reg(c->regs, _A)), 0x00);
    ...
    set_all_flags(c->regs, 0, 0, 0, 0); // Turn carry bit off 
    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +1, 0xdead); /* LD B, 0x00ff */
    mem_write8(c->mem, 0xdead, 0x01); /* LD (HL), d8 */
    mem_write16(c->mem, c->regs->pc +3, 0xcb2e); // SRA (HL)
    run_cpu_loop(c);
    EXPECT_EQ(mem_read8(c->mem, (get_reg_pair(c->regs, _HL))), 0x0000);
    EXPECT_EQ(c->regs->flag, CY_MASK | Z_MASK);
...
TEST(sra2Test, sra2) {
    ...
    cpu *c = new_cpu(m);
    set_all_flags(c->regs, 0, 0, 0, 0);
    mem_write16(c->mem, c->regs->pc, 0x3e01); /* LD A, 0x01 */
    mem_write16(c->mem, c->regs->pc + 2, 0xcb3f); /* SRL A */
    ...
    set_all_flags(c->regs, 0, 0, 0, 0);
    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +1, 0xbeef); /*  0x00ff */
    mem_write8(c->mem, 0xbeef, 0xff); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc + 3, 0xcb3e); /* SRL HL */

    run_cpu_loop(c);
    EXPECT_EQ(mem_read8(c->mem, (get_reg_pair(c->regs, _HL))), 0x007f);
    EXPECT_EQ(c->regs->flag, CY_MASK );
}
...
TEST (bitTest, bit) {
    ...
    set_all_flags(c->regs, 0, 0, 0, 0);
    mem_write8(c->mem, c->regs->pc, 0x21); /* LD HL, d16 */
    mem_write16(c->mem, c->regs->pc +1, 0xbeef); /*  0x00fe */
    mem_write8(c->mem, 0xbeef, 0xfe);
    mem_write16(c->mem, c->regs->pc + 3, 0xcb46); /* BIT 0, HL */
    run_cpu_loop(c);
    EXPECT_EQ(mem_read8(c->mem, (get_reg_pair(c->regs, _HL))), 0xfe);
    EXPECT_EQ(c->regs->flag, Z_MASK | H_MASK);
...
TEST (resTest, res) {
    ...
    mem_write16(c->mem, c->regs->pc, 0x3e01);
    mem_write16(c->mem, c->regs->pc +2, 0xcb87);
    run_cpu_loop(c);
    EXPECT_EQ(*(get_reg(c->regs, _A)), 0x00);

    mem_write16(c->mem, c->regs->pc, 0x3e80); // LD A, 80
    mem_write16(c->mem, c->regs->pc +2, 0xcbbf); // RST 1, A
    run_cpu_loop(c);
    EXPECT_EQ(*(get_reg(c->regs, _A)), 0x00);

    mem_write16(c->mem, c->regs->pc, 0x2e3b); // LD L, 3b
    mem_write16(c->mem, c->regs->pc +2, 0xcb8d); // RST L, 1
    run_cpu_loop(c);
    EXPECT_EQ(*(get_reg(c->regs, _L)), 0x39);
